...
export const monitoredLinks = pgTable("monitored_links", {
  ...
  schedulerLockedUntil: timestamp("scheduler_locked_until", { withTimezone: true }),
  
  // Status tracking
  isActive: boolean("is_active").notNull().default(true),
  currentStatus: text("current_status").$type<"up" | "down" | "degraded" | "pending">().default("pending"),
